...
class QualifyingPredictor:
    ...
    def calibrate_confidence(self, original_confidence: float, 
        ...
        """
        Calibrate the model's internal confidence score to better reflect actual performance.
        
        Args:
            original_confidence: The raw confidence score from the model (0-1)
            evaluation_metrics: Optional dict of evaluation metrics to use for calibration
            calibration_method: Method to use for calibration ('rescale', 'metrics_based', or 'sigmoid')
            
        Returns:
            Calibrated confidence score (0-1)
        """
        if calibration_method == 'rescale':
            # Simple rescaling to make confidence more reasonable
            # This shifts the model's typical confidence range (0.3-0.6) to a more intuitive (0.65-0.95) range
            # Adjust these values based on your model's typical confidence distribution
            min_input = 0.3
            max_input = 0.6
            min_output = 0.65
            max_output = 0.95
            ...
            # Use evaluation metrics to determine confidence
            # Weighted combination of key metrics
            weights = {
                'top5_f1': 0.5,          # How well the model predicts top 5
                'position_within_1': 0.3, # How often we're within 1 position
                'exact_accuracy': 0.2     # How often we get exact position right
            }
            ...
            # Sigmoid transformation to spread confidence values
            # This maps the original confidence through a sigmoid curve
            # centered at 0.5 with steepness controlled by alpha
            import math
            alpha = 10.0  # Steepness factor (higher = steeper curve)
            shifted = original_confidence - 0.5  # Center at 0.5
    ...
    def _format_predictions(self, action_probs: np.ndarray, confidence: float, circuit_name: str) -> Dict:
        ...
        try:
            ...
            # Extract just the top 5 probabilities
            top5_probs = np.array([action_probs[idx] for idx in top5_indices])
            
            # Normalize the top 5 probabilities to sum to 1 (100%)
            ...
            for i, (idx, normalized_prob) in enumerate(zip(top5_indices, top5_probs_normalized), 1):
                ...
                prediction = {
                    ...
                    'original_probability': float(action_probs[idx] * 100),  # Keep original probability for reference
                    'circuit_stats': {
    ...
    def _get_best_position(self, driver_quals: pd.DataFrame) -> int:
        ...
        If no circuit-specific data is available, use reasonable defaults based on driver skill.
        
        Args:
            driver_quals: DataFrame containing qualifying data for this driver at this circuit
            driver_id: The driver's ID
            driver_code: The driver's code (e.g., 'VER', 'HAM')
            
        Returns:
            Best qualifying position (1-20, with 1 being best)
        """
        """Get the driver's best qualifying position at this circuit."""
        if not driver_quals.empty and 'position' in driver_quals.columns:
            # Filter out non-numeric positions
            positions = []
